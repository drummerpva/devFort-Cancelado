...
<HTML>
<HEAD>
<TITLE>Imprima seu boleto de forma rápida!</TITLE>
...
</DIV></table><br><table cellspacing=0 cellpadding=0 width=666 border=0><TBODY><TR><TD class=ct width=666><img height=1 src=imagens/6.png width=665 border=0></TD></TR><TR><TD class=ct width=666><div align=right><b class=cp>Recibo 
...
<table width=666 cellspacing=5 cellpadding=0 border=0 align=Default>
  <tr>
    <td width=50><IMG style="width:100%" SRC="../vendor/img/logo_cartorio.PNG"></td>
...
número</td></tr><tr><td class=cp valign=top width=7 height=12><img height=36 src=imagens/1.png width=1 border=0></td><td class=cp valign=top width=298 height=12> 
  <span class="campo"><?php echo $dadosboleto["cedente"]; ?></span>
  <br/>
  <span class="campo"><?php echo $dadosboleto["cidade_uf"]; ?></span>
  </td>
<td class=cp valign=top width=7 height=12><img height=36 src=imagens/1.png width=1 border=0></td><td class=cp valign=top width=126 height=12> 
  <span class="campo">
...
<td class=cp valign=top width=7 height=12><img height=36 src=imagens/1.png width=1 border=0></td><td class=cp valign=top  width=34 height=12><span class="campo">
  <?php echo $dadosboleto["especie"]?>
</span> 
 </td>
<td class=cp valign=top width=7 height=12><img height=36 src=imagens/1.png width=1 border=0></td><td class=cp valign=top  width=53 height=12><span class="campo">
  <?php echo $dadosboleto["quantidade"]?>
</span> 
 </td>
<td class=cp valign=top width=7 height=12><img height=36 src=imagens/1.png width=1 border=0></td><td class=cp valign=top align=right width=120 height=12> 
  <span class="campo">
  <?php $tmp = $dadosboleto["nosso_numero"];
     //$tmp = substr($tmp,0,strlen($tmp)-1).'-'.substr($tmp,strlen($tmp)-1,1); // COMENTADO PQ NÃO UTILIZO O CALCULO DE NOSSO NUMERO, JA RECEBO CALCULADO
     print $tmp; ?>
  </span></td>
...
<span class="campo">
  <?php
	echo "
		Valor Atualizado: R$ ".number_format($_SESSION['dados'][0]['valorAtualizado'],2,",",".")."&nbsp&nbsp&nbsp&nbsp &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp     
		Desconto: ".number_format(100-($_SESSION['dados'][0]['valorTotal']/$_SESSION['dados'][0]['valorAtualizado'])*100)."%&nbsp&nbsp&nbsp&nbsp &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
		Valor à Pagar: R$ ".number_format($_SESSION['dados'][0]['valorTotal'],2,",",".")."<br><br>";
	
	  $dataPadrao = str_replace("/","-",$_SESSION['dados'][0]['dataVencimento']);
	echo "Vencimento da Proposta: ".$_SESSION['dados'][0]['dataVencimento']."&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp 
		  Com possibilidade de ser pago até: ".date('d/m/Y',strtotime("+7 days",strtotime($dataPadrao)));		
...
de pagamento</td><td class=ct valign=top width=7 height=13><img height=13 src=imagens/1.png width=1 border=0></td><td class=ct valign=top width=180 height=13>Vencimento</td></tr><tr><td class=cp valign=top width=7 height=12><img height=12 src=imagens/1.png width=1 border=0></td><td class=cp valign=top width=472 height=12>
 <?php echo $dadosboleto["textoLocalPagamento"]?></td><td class=cp valign=top width=7 height=12><img height=12 src=imagens/1.png width=1 border=0></td><td class=cp valign=top align=right width=180 height=12> 
  <span class="campo">
  <?php echo $dadosboleto["data_vencimento"]?>
  </span></td>
...
cedente</td></tr><tr><td class=cp valign=top width=7 height=12><img height=12 src=imagens/1.png width=1 border=0></td><td class=cp valign=top width=472 height=12> 
  <span class="campo">
  <?php echo $dadosboleto["cedente"]." - CNPJ: ".$dadosboleto["cpf_cnpj"]." - ".$dadosboleto["cidade_uf"]?>
  </span></td>
<td class=cp valign=top width=7 height=12><img height=12 src=imagens/1.png width=1 border=0></td><td class=cp valign=top align=right width=180 height=12> 
  <span class="campo">
  <?php $tmp2 = $dadosboleto["codigo_cliente"];
...
<?php echo "Não receber valores diferentes dos citados abaixo:";?>
<table class="ti" cellspacing="5px" style="text-align: center">
	<tr>
...
<?php 
	foreach($_SESSION['dados'] as $rst){
		if($rst['condicao'] == '01'){
			echo "
				<tr>
					<td>À Vista</td>
					<td>R$ ".number_format($rst['valorTotal'],2,',','.')."</td>
					<td>********</td>
					<td>R$ ".number_format($rst['valorTotal'],2,',','.')."</td>
				</tr>
			";
		}/*elseif($rst['condicao'] == '02'){
			echo "
				<tr>
					<td>1 + 1</td>
					<td>R$ ".number_format($rst['valorTotal'],2,',','.')."</td>
					<td>********</td>
					<td>R$ ".number_format(($rst['valorTotal']*2),2,',','.')."</td>
				</tr>
			";
			
		}*/else{
			$parcelas = (integer) $rst['condicao'];
			echo "
				<tr>
					<td>1 + ".($parcelas-1)."</td>
					<td>R$ ".number_format($rst['valorTotal'],2,',','.')."</td>
					<td>R$ ".number_format($rst['valorTotal'],2,',','.')."</td>
					<td>R$ ".number_format(($rst['valorTotal']*$parcelas),2,',','.')."</td>
				</tr>
			";
...
<?php
	$dataPadrao = str_replace("/","-",$_SESSION['dados'][0]['dataVencimento']);
	echo "Boleto válido para pagamento até: ".date('d/m/Y',strtotime("+7 days",strtotime($dataPadrao)));
?>
